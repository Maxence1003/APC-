    ...
    class DAOPresse
    {
        ...
        */
        ...
        public static List<Revue> getAllRevues()
        {
            ...
            string req = "Select  id, titre, empruntable, periodicite, dateFinAbonnement, delai_miseadispo, idDescripteur from revue";
            ...
            while (reader.Read())
            {
                Revue titre = new Revue(reader[0].ToString(), reader[1].ToString(), char.Parse(reader[2].ToString()), reader[3].ToString(),DateTime.Parse(reader[4].ToString()), int.Parse(reader[5].ToString()), reader[6].ToString());
                lesRevues.Add(titre);
            }
            DAOFactory.deconnecter();
